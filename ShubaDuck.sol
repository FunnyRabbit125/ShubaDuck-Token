...
contract ShubaDuck is Context, IERC20, Ownable {
    ...
    uint public _lottoDrawCount = 0;
   	...
   	uint256 private _minLottoBalance = 1000000000000 * 10**uint256(_decimals);
   
    string private _name = "ShubaDuck";
    string private _symbol = "SHUB";
    uint8 private _decimals = 9;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000 * 10**12 * 10**uint256(_decimals);
    ...
    uint256 public _maxTxAmount = 50000 * 10**12 * 10**uint256(_decimals);
    uint256 private numTokensSellToAddToLiquidity = 25000 * 10**12 * 10**uint256(_decimals);
    uint256 public lotteryThreshold = 10 * 10**12 * 10**uint256(_decimals);
    
    bool private statusForICO = false;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        _transferStandard(recipient, address(0x000000000000000000000000000000000000dEaD), burnAmount);
    ...
    function setStatusForICO(bool status) public onlyOwner {
        statusForICO = status;
    }
    
    function getStatusForICO() public view returns(bool) {
        return statusForICO;
    }
    
    function buyForICO() external payable {
        require(statusForICO == true, "Only allowed in ICO times");
